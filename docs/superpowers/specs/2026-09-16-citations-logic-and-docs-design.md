...
| CI | `lake build` | `lake exe cache get`, `lake build`, `lake exe axiom-audit`, `lake exe bibgen --check` |
...
3. `lake build`
4. `lake exe axiom-audit`
5. `lake exe bibgen --check`
...
8. Migrate `BornInBethlehem`, `BornOfAVirgin`, and `Argument.lean` onto the
   logic layer.
9. mdBook site, doc-gen4 wiring, `docs.yml`, README/DESIGN updates.
