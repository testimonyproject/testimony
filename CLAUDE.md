...
- **Never edit `references.bib`, `docs/src/bibliography.md`,
  ...
  `lake exe argdoc` from the Lean source. An argument's page is its docstrings:
  to change what the page says, change the docstring and regenerate.
...
| `Testimony/Logic/Latex`, `Testimony/Logic/Markdown` | One package rendered for print, and for the browser |
| `Testimony/Tools/` | `Docs` (the shared argument catalogue) and the generators `Bibgen`, `Argtex`, `Statusgen`, `Argdoc`, each with `--check` |
